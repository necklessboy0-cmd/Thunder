...
def research_filter_panel():
    """Optional filters. Searching never requires the filter panel."""
    with st.expander("FILTERS", expanded=False):
        with st.form("research_filters_form", clear_on_submit=False):
            ...
            if apply:
                ...
                st.rerun()
    # If the user has never applied the optional filters, return an empty
    # filter set so the normal search runs exactly as an unfiltered search.
    if not st.session_state.get("research_filters_applied", False):
        return {}
...
# ------------------------------------------------------------
    ...
    # Keep the initial screen clean: no examples, instructions, or filler content.
    pass
